...
static unsigned char ebc2ascArray[256] = {
...
static unsigned char asc2ebcArray[256] = {
...
int16_t asc2ebc(char * pstr, const unsigned int *plength){
	#ifdef DEBUG
		fprintf(stdout, "About to convert ascii2ebcdic !! \n");
	#endif 	  
	
	int i = 0;
	for (i = 0; i < *plength; i++){
	    *(pstr + i ) = ASCIItoEBCDIC ( *(pstr + i ) ) ;
	#ifdef DEBUG
		fprintf(stdout, "%c", *(pstr + i) );
	#endif   
	 }   
	#ifdef DEBUG
		fprintf(stdout, "\n" );
...
int16_t ebc2asc(char * pstr, const unsigned int *plength){
	#ifdef DEBUG
		fprintf(stdout, "About to convert ebcdic2ascii !! \n");
	#endif 	  
	
	int i = 0;
	for (i = 0; i < *plength; i++){
	    *(pstr + i ) = EBCDICtoASCII ( *(pstr + i ) ) ;
	#ifdef DEBUG
		fprintf(stdout, "%c", *(pstr + i) );
	#endif   
	 }   
...
int16_t inithexa(char * pstr, const unsigned int *plength){
	#ifdef DEBUG
		fprintf(stdout, "About to start inithexa !! \n");
	#endif 	  
